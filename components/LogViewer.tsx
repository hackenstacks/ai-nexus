...
export const LogViewer: React.FC<LogViewerProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 z-40 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-nexus-gray-800 rounded-lg shadow-xl w-full max-w-4xl h-full max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <header className="p-4 border-b border-nexus-gray-700 flex justify-between items-center flex-shrink-0">
          ...
          <div className="flex items-center space-x-4">
            <button onClick={logger.clearLogs} className="flex items-center space-x-2 text-nexus-gray-400 hover:text-white transition-colors">
        ...
        <div ref={logContainerRef} className="flex-1 p-4 overflow-y-auto font-mono text-sm space-y-2">
          {logs.map(log => (
            <div key={log.id} className="border-b border-nexus-gray-700/50 pb-2">
              <div className="flex items-baseline flex-wrap">
                <span className="text-gray-500 mr-2">{log.timestamp.toLocaleTimeString('en-US', { hour12: false })}</span>
                <span className={`font-bold mr-2 px-2 py-0.5 rounded text-xs ${levelClasses[log.level].bg} ${levelClasses[log.level].text}`}>{log.level}</span>
                <span className="text-nexus-gray-200 whitespace-pre-wrap">{log.message}</span>
              </div>
              {log.details && (
                <pre className="text-nexus-gray-400 text-xs bg-nexus-dark p-2 rounded mt-1 whitespace-pre-wrap break-words">
