    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 z-40 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-nexus-gray-light-100 dark:bg-nexus-gray-800 rounded-lg shadow-xl w-full max-w-lg flex flex-col" onClick={e => e.stopPropagation()}>
        <header className="p-4 border-b border-nexus-gray-light-300 dark:border-nexus-gray-700 flex justify-between items-center flex-shrink-0">
          <h2 className="text-xl font-bold text-nexus-gray-900 dark:text-white">Start a New Chat</h2>
        ...
        <div className="p-6 space-y-4">
            <div>
                <label htmlFor="chat-name" className="block text-sm font-medium text-nexus-gray-800 dark:text-nexus-gray-300">Chat Name</label>
                <input
                    id="chat-name"
                    type="text"
                    value={chatName}
                    ...
                    required
                    className="mt-1 block w-full bg-nexus-gray-light-200 dark:bg-nexus-gray-700 border border-nexus-gray-light-400 dark:border-nexus-gray-600 rounded-md shadow-sm py-2 px-3 text-nexus-gray-900 dark:text-white placeholder-nexus-gray-600 dark:placeholder-nexus-gray-400 focus:outline-none focus:ring-nexus-blue-500 focus:border-nexus-blue-500"
                />
            </div>

            <div>
                 <label className="block text-sm font-medium text-nexus-gray-800 dark:text-nexus-gray-300">Select Characters ({selectedIds.size})</label>
                 <div className="mt-2 max-h-60 overflow-y-auto border border-nexus-gray-light-300 dark:border-nexus-gray-700 rounded-md p-2 space-y-2">
                    {characters.length === 0 ? (
                        <p className="text-nexus-gray-700 dark:text-nexus-gray-400 text-center p-4">No characters found. Please create one first.</p>
                    ) : characters.map(character => (
                        <div key={character.id} onClick={() => handleToggleCharacter(character.id)} className={`flex items-center p-2 rounded-md cursor-pointer transition-colors ${selectedIds.has(character.id) ? 'bg-nexus-blue-600/50' : 'hover:bg-nexus-gray-light-300 dark:hover:bg-nexus-gray-700'}`}>
                            <input
                                type="checkbox"
                                checked={selectedIds.has(character.id)}
                                readOnly
                                className="h-4 w-4 rounded border-nexus-gray-500 dark:border-nexus-gray-600 bg-nexus-gray-light-100 dark:bg-nexus-gray-900 text-nexus-blue-500 focus:ring-nexus-blue-500 pointer-events-none"
                            />
                            ...
                            <span className="font-medium text-nexus-gray-900 dark:text-white">{character.name}</span>
                        </div>
                    ))}
                 </div>
            </div>
        ...
        <footer className="p-4 border-t border-nexus-gray-light-300 dark:border-nexus-gray-700 flex justify-end space-x-3">
