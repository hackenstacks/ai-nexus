    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 z-40 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-nexus-gray-800 rounded-lg shadow-xl w-full max-w-lg flex flex-col" onClick={e => e.stopPropagation()}>
        <header className="p-4 border-b border-nexus-gray-700 flex justify-between items-center flex-shrink-0">
          <h2 className="text-xl font-bold text-white">Import Memory</h2>
        ...
        <div className="p-6">
            <p className="text-nexus-gray-300 mb-4">Select a chat session to import memories from. Any characters present in both chats will have their memories from the selected session appended to their current memory.</p>
            
            <div className="max-h-80 overflow-y-auto border border-nexus-gray-700 rounded-md p-2 space-y-2">
                {otherSessions.length === 0 ? (
                    <p className="text-nexus-gray-400 text-center p-4">No other chat sessions available to import from.</p>
                ) : (
                    otherSessions.map(session => (
                        <div 
                            key={session.id} 
                            onClick={() => onImport(session.id)}
                            className="p-3 rounded-md cursor-pointer hover:bg-nexus-gray-700 transition-colors"
                        >
        ...
        <footer className="p-4 border-t border-nexus-gray-700 flex justify-end">
